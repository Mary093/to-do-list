...
// Lista separata per i film salvati dall'utente
let savedFilms = [];

// Inizializzazione della pagina quando il DOM è caricato
...
function setupNavigation() {
    const exploreLink = document.querySelector('a[href="#"]:first-of-type');
    const myListsLink = document.querySelector('a[href="#"]:nth-of-type(2)');
...
function showExploreSection() {
    ...
    container.innerHTML = `
        <header class="mb-4">
            <h2 class="text-dark"> Esplora Film</h2>
            <div class="d-flex gap-2">
                <input type="text" id="searchInput" class="form-control search-input" placeholder="Cerca film..." />
                <button class="btn btn-secondary" id="searchBtn">
                    <span class="material-icons me-1">search</span> Cerca
                </button>
            </div>
        </header>
        <div>
            <div id="searchResults" class="row mb-5"></div>
            <h4 class="text-dark">🎬 Tutti i Film Disponibili</h4>
            <div id="exploreList" class="row"></div>
        </div>
    `;
...
function showMyListsSection() {
    ...
    container.innerHTML = `
        <header class="mb-4">
            <h2 class="text-dark"> Le mie Liste</h2>
        </header>
        <div>
            <div class="mb-4">
                <h4 class="text-dark">➕ Aggiungi Nuovo Film</h4>
                <div class="row">
                    <div class="col-md-3">
                        <input type="text" id="titolo" class="form-control mb-2" placeholder="Titolo" />
                    </div>
                    <div class="col-md-3">
                        <input type="text" id="genere" class="form-control mb-2" placeholder="Genere" />
                    </div>
                    <div class="col-md-2">
                        <input type="number" id="anno" class="form-control mb-2" placeholder="Anno" min="1900" max="2024" />
                    </div>
                    <div class="col-md-2">
                        <input type="text" id="durata" class="form-control mb-2" placeholder="Durata (es. 120 min)" />
                    </div>
                    <div class="col-md-2">
                        <button onclick="addFilmFromInput()" class="btn btn-primary mb-2 w-100">
                            <span class="material-icons me-1">add</span> Aggiungi
                        </button>
                    </div>
                </div>
                <div class="row">
                    <div class="col-12">
                        <textarea id="descrizione" class="form-control mb-2" placeholder="Descrizione del film" rows="2"></textarea>
                    </div>
                </div>
            </div>
            <h4 class="text-dark">️ Film Salvati (${savedFilms.length})</h4>
            <div id="savedList" class="row"></div>
        </div>
    `;
...
// Funzione per renderizzare i film nella sezione Esplora
function renderExploreFilms() {
    ...
    films.forEach(film => {
        ...
        htmlContent += `
            <div class="film-list-item d-flex">
                <img src="${film.locandina}" class="film-poster" alt="${film.titolo}">
                <div class="film-info">
                    <div>
                        <h5 class="film-title">${film.titolo} (${film.anno})</h5>
                        <div class="film-details">
                            <p class="mb-1"><strong>Genere:</strong> ${film.genere}</p>
                            <p class="mb-1"><strong>Durata:</strong> ${film.durata}</p>
                        </div>
                        <p class="film-description">${film.descrizione}</p>
                    </div>
                    <div class="film-actions">
                        <button class="btn ${isSaved ? 'btn-success' : 'btn-primary'}" 
                                onclick="toggleSavedFilm(${film.id})">
                            <span class="material-icons me-1">${isSaved ? 'bookmark' : 'bookmark_border'}</span>
...
// Funzione per renderizzare i film salvati
        ...
        htmlContent = `
            <div class="col-12">
                <div class="alert alert-info">
                    <h5>Nessun film salvato</h5>
                    <p>Vai nella sezione "Esplora" per aggiungere film alla tua lista!</p>
                </div>
            </div>
        `;
    } else {
            ...
            htmlContent += `
                <div class="film-list-item d-flex">
                    ...
                    <div class="film-info">
                        ...
                        <div class="film-actions">
                            <button class="btn btn-danger" onclick="removeSavedFilm(${film.id})">
...
function toggleSavedFilm(filmId) {
    ...
    // Aggiorna la visualizzazione se siamo nella sezione Esplora
...
function performSearch() {
    ...
    if (filteredFilms.length > 0) {
        htmlContent = `<h4 class="text-dark">🔍 Risultati della ricerca (${filteredFilms.length})</h4>`;
...
function addFilm(locandina, titolo, genere, anno, durata, descrizione) {
    const newFilm = {
        id: Date.now(), // Usa timestamp per ID univoco
        locandina: locandina,
        titolo: titolo,
        genere: genere,
        anno: anno,
...
function addFilmFromInput() {
    ...
    const genere = genereInput.value.trim();
    const anno = annoInput.value.trim();
    const durata = durataInput.value.trim();
